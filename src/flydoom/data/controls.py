...
def erdos_renyi_matched(graph: ConnectomeGraph, seed: int) -> ConnectomeGraph:
    ...
    ids = graph.neurons["body_id"].to_numpy()
    weights = rng.permutation(graph.edges["synapse_count"].to_numpy())
    edges = pd.DataFrame(
        {
            "pre_body_id": ids[sources],
            "post_body_id": ids[targets],
...
def degree_preserving_rewire(
    ...
    rng = np.random.default_rng(seed)
    pairs = list(graph.edges[["pre_body_id", "post_body_id"]].itertuples(index=False, name=None))
    edge_set = set(pairs)
    ...
    while successful < target_swaps and attempts < max_attempts:
        ...
        successful += 1
    edges = graph.edges.copy()
    edges["pre_body_id"] = [pair[0] for pair in pairs]
    edges["post_body_id"] = [pair[1] for pair in pairs]
    return ConnectomeGraph(graph.neurons.copy(), edges)
